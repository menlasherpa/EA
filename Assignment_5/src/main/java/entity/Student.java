...
@NamedQuery(name = "Student.findStudents", query = "SELECT s FROM Student s WHERE s.gpa >= 3 AND SIZE(s.courseAttended) >= 9 AND s.campusCourseAttending IS NULL AND s.distanceCourseAttending IS NULL", lockMode = LockModeType.PESSIMISTIC_WRITE)
@Entity
@Cacheable
@EntityListeners({PersistListener.class, DeleteListener.class})
